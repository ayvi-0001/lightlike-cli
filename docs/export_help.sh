...
_export_help() {
    for cmd in "${commands[@]}"; do
        python -m lightlike $cmd --help || return # return if failed to acquire lock on 1st attempt
        mv $(echo "$cmd" | sed -r 's/\s+/_/g').svg docs/assets/svg/
    done
}

commands=(
    "app" "app sync" "app run-bq" "app dir" "app config" "app config show" "app config open" "app config set"
    ...
    "summary" "summary table" "summary json" "summary csv"
